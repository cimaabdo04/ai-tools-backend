...
import { ModelsInput } from "@components/ui/models-input";
import { StartStepsInput } from "@components/ui/start-steps-input";
import { StatsInput } from "@components/ui/stats-input";
import { ErrorState } from "@components/common/error-state";
import { FaqManager } from "@components/tools/faq-manager";
import { Save, ExternalLink, X, Plus, Upload, Hash, CheckSquare, ArrowLeft, Github, MessageCircle } from "lucide-react";
import Link from "next/link";

const pricingOptions = [
  { value: "free", label: "مجاني" },
  { value: "freemium", label: "فريميوم" },
...
export default function DashboardToolEdit() {
  ...
  const updateMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["dashboard", "edits", "tool", id] });
      queryClient.invalidateQueries({ queryKey: ["dashboard", "edits"] });
  ...
  const tool = rawTool;
  const tagsList = (tagsData as any)?.data || [];
  const categoriesList = (categoriesData as any)?.data?.categories || [];
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-bold tracking-tight">تعديل: {tool.name}</h1>
      ...
      {updateMutation.isError && (
        <div className="rounded-lg bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 p-4 text-sm text-red-700 dark:text-red-300">
          فشل الحفظ. تحقق من الصلاحيات أو حاول مرة أخرى.
        </div>
      )}
      ...
      <div className="grid gap-6 lg:grid-cols-[2fr_1fr]">
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            <ImageUpload label="الشعار" value={form.logo !== undefined ? form.logo : tool.logo} onChange={(url) => setFormField("logo", url)} />
            <ImageUpload label="لقطة الشاشة" value={form.screenshot !== undefined ? form.screenshot : tool.screenshot} onChange={(url) => setFormField("screenshot", url)} />
            ...
            <div>
              ...
              <Input defaultValue={tool.videoUrl || ""} onChange={(e) => setFormField("videoUrl", e.target.value)} placeholder="https://youtube.com/watch?v=..." dir="ltr" />
            </div>
          </CardContent>
        </Card>
      </div>
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            <div>
              ...
              <Select
                options={[{ value: "", label: "اختر تصنيف" }, ...categoriesList.map((c: any) => ({ value: c.id, label: c.name }))]}
                defaultValue={String(tool.category?.id || "")}
          ...
          <CardContent className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <Select options={pricingOptions} defaultValue={Array.isArray(tool.pricingModel) ? tool.pricingModel[0] : String(tool.pricingModel || "free")} onChange={(e) => setFormField("pricingModel", e.target.value)} />
              </div>
              <div>
                <label className="text-sm font-medium">السعر يبدأ من $</label>
                <Input type="number" min={0} step={0.01} defaultValue={tool.pricingStartingAt ?? ""} onChange={(e) => setFormField("pricingStartingAt", Number(e.target.value) || 0)} />
              </div>
            </div>
          </CardContent>
        </Card>
      ...
      <Card>
        ...
        <CardContent className="space-y-6">
          <div>
            ...
            <div className="flex gap-2 mb-2">
              <Input value={featureInput} onChange={(e) => setFeatureInput(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter") { e.preventDefault(); addFeature(); } }} placeholder="أضف ميزة..." className="flex-1" />
        ...
        <CardContent className="space-y-4">
          <div className="grid gap-4 sm:grid-cols-2">
            <div>
              ...
              <Input defaultValue={tool.twitterUrl || ""} onChange={(e) => setFormField("twitterUrl", e.target.value)} placeholder="https://twitter.com/..." dir="ltr" />
            </div>
            <div>
              <label className="text-sm font-medium">ديسكورد</label>
              <Input defaultValue={tool.discordUrl || ""} onChange={(e) => setFormField("discordUrl", e.target.value)} placeholder="https://discord.gg/..." dir="ltr" />
            </div>
            <div>
              <label className="text-sm font-medium">GitHub</label>
              <Input defaultValue={tool.githubUrl || ""} onChange={(e) => setFormField("githubUrl", e.target.value)} placeholder="https://github.com/..." dir="ltr" />
            </div>
          </div>
        </CardContent>
      </Card>
        ...
        <CardContent className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <Select options={statusOptions} defaultValue={String(tool.status || "pending")} onChange={(e) => setFormField("status", e.target.value)} />
            </div>
          </div>
          <div className="flex items-center justify-between">
            <div>
            ...
            <Switch checked={form.featured !== undefined ? form.featured : tool.featured} onCheckedChange={(checked) => setFormField("featured", checked)} />
          </div>
          <div className="flex items-center justify-between">
            <div>
            ...
            <Switch checked={form.verified !== undefined ? form.verified : tool.verified} onCheckedChange={(checked) => setFormField("verified", checked)} />
          </div>
        </CardContent>
      </Card>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-base">قسم البدائل</CardTitle>
        </CardHeader>
        <CardContent className="space-y-4">
          <div>
            <label className="text-sm font-medium">نص البدائل (alternativesText)</label>
          ...
          <div>
            <label className="text-sm font-medium">الأدوات البديلة (alternativeSlugs)</label>
            <ToolSearchSelect value={form.alternativeSlugs !== undefined ? form.alternativeSlugs : tool.alternativeSlugs || '[]'} onChange={(v) => setFormField("alternativeSlugs", v)} />
          </div>
        </CardContent>
      </Card>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-base">كيف تبدأ؟</CardTitle>
        </CardHeader>
        <CardContent className="space-y-4">
          <div>
            <label className="text-sm font-medium">خطوات البدء (startSteps)</label>
            <StartStepsInput value={form.startSteps !== undefined ? form.startSteps : tool.startSteps || '[]'} onChange={(v) => setFormField("startSteps", v)} />
          </div>
        </CardContent>
      </Card>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-base">الخلاصة</CardTitle>
        </CardHeader>
        <CardContent>
          <div>
            <label className="text-sm font-medium">نص الخلاصة (conclusion)</label>
            <Textarea
              defaultValue={tool.conclusion || ""}
              rows={3}
              onChange={(e) => setFormField("conclusion", e.target.value)}
              placeholder="نص الخلاصة في نهاية المقال..."
            />
          </div>
        </CardContent>
      </Card>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-base">الشارة والنقاط البارزة</CardTitle>
        </CardHeader>
        <CardContent className="space-y-4">
          <div>
            <label className="text-sm font-medium">الشارة (badge)</label>
            <Input defaultValue={tool.badge || ""} onChange={(e) => setFormField("badge", e.target.value)} placeholder="مثال: الرائد الاقتصادي 2026" />
          </div>
          <div>
            <label className="text-sm font-medium">النقطة المميزة (highlight)</label>
            <Textarea defaultValue={tool.highlight || ""} rows={2} onChange={(e) => setFormField("highlight", e.target.value)} placeholder="نص تسويقي مميز يظهر في صندوق info أعلى المقال" />
          </div>
          <div>
            <label className="text-sm font-medium">الإحصائيات (stats)</label>
            <StatsInput value={form.stats !== undefined ? form.stats : tool.stats || '{}'} onChange={(v) => setFormField("stats", v)} />
          </div>
        </CardContent>
      </Card>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-base">النماذج (Pro / Flash)</CardTitle>
        </CardHeader>
        <CardContent className="space-y-4">
          <div>
            <label className="text-sm font-medium">النماذج (models)</label>
            <ModelsInput value={form.models !== undefined ? form.models : tool.models || '[]'} onChange={(v) => setFormField("models", v)} />
          </div>
        </CardContent>
      </Card>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-base">حالات الاستخدام والجمهور</CardTitle>
        </CardHeader>
        <CardContent className="space-y-4">
          <div>
            <label className="text-sm font-medium">حالات الاستخدام (useCases)</label>
            <Textarea
              defaultValue={(() => { try { return JSON.parse(tool.useCases || '[]').join('\n'); } catch { return ''; } })()}
              rows={4}
              onChange={(e) => setFormField("useCases", JSON.stringify(e.target.value.split('\n').filter(Boolean)))}
              placeholder="كل حالة استخدام في سطر منفصل"
            />
          </div>
          <div>
            <label className="text-sm font-medium">الجمهور المستهدف (targetAudience)</label>
            <Textarea
              defaultValue={(() => { try { return JSON.parse(tool.targetAudience || '[]').join('\n'); } catch { return ''; } })()}
              rows={4}
              onChange={(e) => setFormField("targetAudience", JSON.stringify(e.target.value.split('\n').filter(Boolean)))}
              placeholder="كل فئة في سطر منفصل"
            />
          </div>
        </CardContent>
      </Card>
