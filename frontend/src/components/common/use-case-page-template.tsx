...
import type { UseCasePageConfig } from "@/data/use-case-pages";

function UseCasePageContent({ config }: { config: UseCasePageConfig }) {
  ...
  const filterValue = config.useCaseFilter.toLowerCase();
  const tools = (allTools?.data ?? [])
    .filter((t) =>
      t.useCases?.some((uc) => uc.toLowerCase().includes(filterValue))
    )
    .slice(0, 10);
      ...
      <div className="container pb-12">
        {isLoading ? (
          ...
          <div className="text-center py-20">
            <p className="text-muted-foreground">لا توجد أدوات متاحة حالياً لهذا الاستخدام.</p>
          </div>
        ) : (
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex items-center gap-3 mt-4">
                            <Button size="sm" asChild>
                              <Link href={ROUTES.TOOL_DETAIL(tool.slug)}>عرض التفاصيل</Link>
                            </Button>
                            {tool.websiteUrl && (
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <Card className="hover:shadow-md hover:border-primary/50 transition-all">
            <CardContent className="p-6 text-center">
              ...
              <h3 className="font-semibold mb-1">الأكثر تميزاً</h3>
              <p className="text-sm text-muted-foreground mb-4">
                تصفح أفضل أدوات الذكاء الاصطناعي
              </p>
              <Button variant="outline" size="sm" asChild>
                <Link href={ROUTES.BEST.AI_TOOLS}>أفضل الأدوات</Link>
              </Button>
            </CardContent>
          </Card>
          <Card className="hover:shadow-md hover:border-primary/50 transition-all">
...
export function UseCasePageTemplate({ config }: UseCasePageTemplateProps) {
  return <UseCasePageContent config={config} />;
}
